...
func NewZillow(zwsId string, debug bool) Zillow {
	return &zillow{
		zwsId: zwsId,
		url:   baseUrl,
		debug: debug,
...
type ZestimateResult struct {
	...
	RentZestimate   RentZestimate      `xml:"response>rentzestimate"`
...
type SearchResult struct {
	...
	RentZestimate   RentZestimate      `xml:"rentzestimate"`
...
func (z *zillow) get(path string, values url.Values, result interface{}) error {
	sURL := z.url + "/" + path + ".htm?" + values.Encode()
	if z.debug {
		log.Printf("zillow get url: %s", sURL)
	}
	if resp, err := http.Get(sURL); err != nil {
		return err
...
func (z *zillow) GetZestimate(request ZestimateRequest) (*ZestimateResult, error) {
	...
	if z.debug {
		log.Printf("zillow get url: %s, values: %v", zestimatePath, values)
	}
...
func (z *zillow) GetSearchResults(request SearchRequest) (*SearchResults, error) {
	...
	if z.debug {
		log.Printf("zillow get url: %s, values: %v", searchResultsPath, values)
	}
